...
\chapter{Bubble Nucleation 40\%}\label{ch:nucleation}
%\begin{quote}
%\flushleft
...
We shall investigate the validity of the capillary approximation for the case of water and perfluoropentane momentarily, 
...
\begin{figure}
  \input{part_theoretical/nucleation/figs/chNucDissolveTimeLum.tex}
  \caption{ Dissolve times  }
 \label{fig:nuc:dissolve_time_lum}
\end{figure}


\begin{figure}
  \input{part_theoretical/nucleation/figs/chNucDissolveTime.tex}
  \caption{ Dissolve times  }
 \label{fig:nuc:dissolve_time}
\end{figure}


\begin{figure}
  \input{part_theoretical/nucleation/figs/chNucDissolveTimeLumPFP.tex}
  \caption{ Dissolve times  }
 \label{fig:nuc:dissolve_time_lum_PFP}
\end{figure}


\begin{figure}
  \input{part_theoretical/nucleation/figs/chNucDissolveTimePFP.tex}
  \caption{ Dissolve times  }
 \label{fig:nuc:dissolve_time_PFP}
\end{figure}


\ctable[cap=Dissolution symbols
        caption=Symbols used in the calculation of dissolution times
        label=table:nuc:dissolve_vars
        pos=top
        %width = 0.6\textwidth,,,
        left
       ]
...
It was seen that bubbles sizes in the tens of nanometres can be stable when placed under 
tension by the rarefactional cycles of an acoustic wave.
However, the lifetime of the bubble after the rarefaction has passed was not given.
This is the goal of this section.

This section does not consider the generation of the bubble at all.
While we imagine the bubble to be generated in a rarefaction cycle of an acoustic wave
we will not in fact model the wave at all.
Rather, a bubble is placed in the medium 
This model places a bubble that was stable at one 

In this section a very simple model is assumed.
A passing sound wave is assumed to generate a bubble in the fluid in its rarefactional cycle.
However, the interaction between the sound wave and the bubble will be assumed to be quasi static, 
so that no pulsatons in the bubble's volume will be excited.
Then, when the sound wave passes, the bubble returns to the medium that is unchanged by the passage of the wave,
and again no pulsations are induced in the bubble.

The ambient pressure of the medium is denoted   $P_\infty$ and the ambient density of the gas in the medium is  $n_\infty$.

The Concetration at bubble, $n_R$ wall will vary as air moves from the bubble to the solution.

In general, pressure at the bubble wall
\begin{align}
  P_R = P_\infty + \frac{2\sigma}{R} 
\end{align}
and Henry's law implies proportionality between the number density and the pressure:
\begin{align}
  \frac{n_R}{n} = \frac{P_R}{P}.
\end{align}


Will a bubble be me
Follow the argument of Gor\cite{Gor2011}

For a recent discussion of the dissolation of a bubble with a permeable shell, see Sarkar\cite{Sarkar2009}.
...
\nlist{
\item
The first is when an acoustic rarefaction first passes through the liquid and nucleates  
a bubble.  
When the wave has passed the bubble will be at the ambient pressure $P_\infty$ and in a fluid where the  ambient concentration of the gas is unchanged at  $n_\infty$ 

air bubble dropped into ambient satuated solution, in which case
  \begin{align}
    n_0 &= n
  \end{align}
\item air bubble dropped into unsaturated solution, in which case
   \begin{align}
    n_0 &= 0
  \end{align}
}
...
Introduce dimensionless measures of saturation and solubility,
\begin{align}
...
\end{align}
Two cases of interest are therefore $\zeta = 0$ and $\zeta = -1$, although solve general case.

Assume Fickian diffusion of gas
\begin{align}
  j_D = -D \frac{\partial n}{\partial r}
\end{align}
where spherical symmetric function of radius, $r$.

This can be solved via the diffusion equation (neglecting transport term) 
so that
\begin{align}
  \frac{\partial n}{\partial r} = \lr{n_R - n_0}\lrsquare{\frac{1}{R} + \frac{1}{\sqrt{\pi D t}}}
\end{align}
(see Epstein and Plesset for details\cite{Epstein1950}).

Material conservation requires
\begin{align}
  \frac{dN}{dt} =4 \pi R^2 j_D.
\label{eqn:nuc:materialCons}
\end{align}

Assuming the contents of the bubble is an ideal gas, 
\begin{align}
  N = \frac{4\pi R^3 }{3\kB T} \lrsquare{P_\infty + \frac{2\sigma}{R}}
\end{align}
which when differentiated and equated to \eqnref{nuc:materialCons} gives
\begin{align}
   \dot{R}\lrsquare{ 1+ \frac{4\sigma}{P_\infty R}} = Ds\lrsquare{ \zeta - \frac{2\sigma}{P_\infty R}}\lrsquare{\frac{1}{R} + \frac{1}{\sqrt{\pi D t}}}
\end{align}
where we have used equation \eqnref{nuc:solubility}
\begin{align}
  \frac{\lr{n_0 - n_R}}{n_\infty} = \zeta - \frac{2\sigma}{P_\infty R}
  \label{eqn:nuc:dissolve}
\end{align}
which follows from \eqnref{nuc:saturation}.

The numerical solution of \eqnref{nuc:dissolve} is plotted for a 1 micron bubble in \figref{nuc:dissolve_time_lum}.
The dissolution time (when $R = 0$) is plotted against the initial radius in \figref{nuc:dissolve_time}.



\subsection{The validity of the capillary approximation}\label{sec:nuc:DFT}
%\begin{quote}
...
indicating a very low interaction length (illustrated in \figref{nuc:LennardJones}.
...
%To begin this study, this chapter asks the following questions:
%
%\subsection{Structure of the chapter}
%
