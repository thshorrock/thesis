...
\ctable[cap=Boiling points and critical temperatures of various perfluorocarbons
        ...
        label=table:nuc:critical_temps
        pos=top
        %width = 0.6\textwidth,
        left
       ]
...
This series of oils is characterised by their low boiling points, given in \tableref{nuc:critical_temps}, and  their high solubility of many gases.
...
and is explored in detail in \secref{nuc:evacuation}.
...
\Secref{nuc:radius} then completes the calculations of \secref{nuc:evacuate} and \secref{vapourise}
...
It is greater than the limit of observation typically chosen in other nucleation applications: \unit{1}\centi\metre\rpcubed\reciprocal\second.\todo{cite}.
...
\begin{align}
J = J_0\exp \lr{-\frac{\Delta G}{\kB T}},% = J_0 \exp\lr{- \frac{16\pi\gamma^3}{3\kB T\lr{p_v^\ast - p_L}^2}},
\label{eqn:nuc:J}
\end{align}
...
\ctable[cap=Nucleation dimensionless groups
        ...
        label=table:nuc:dimensional_vars
        pos=top
        %width = 0.6\textwidth,
        left
       ]
...
The distance over which the density changes from liquid to vapour becomes not insignificant %for very small bubbles
...
\subsection{Density Functional theory}\label{sec:nuc:DFT}
%\begin{quote}
...
The critical temperatures for a number of perfluorocarbons are listed in \tabref{nuc:critical_temps}.
...
In short, the algorithm we shall be using proceeds as follows % The steps that are to be taken are 
 \nlist{
   \item write down an accurate model for the intermolecular potential
   \item approximate the model so that it can be solved.
   \item fit the model's parameters so that it reproduces macroscopic thermodynamics
   \item use thermodynamic arguments to extend the predictions to other parameters.
 }



\subsubsection{Approximating the grand potential}

The grand potential $\Omega$ is required to evaluate both the rate of nucleation and the critical radius.
%If  spherical symmetry is assumed then 
%the bubble boundary is defined by its radius.
%The critical radius is such that\cite{Oxtoby1992,Oxtoby1988}
%\begin{align}
...
The condition \eqnref{DFT:astarR}
...
To use equation \eqnref{ChemPots}, however, 
we first need to evaluate the bulk properties about which we construct the perturbation.
Only then can we find $\mu$, the `mean field' guess for the iteration. %necessary for the right hand side of \eqnref{ChemPots}.
...
Equation \Eqnref{OmegaVBulk}  can be minimised with the help of the Maxwell relation
\begin{align}
...
\end{align}
Substituting \eqnref{muBulk} into \eqnref{OmegaVBulk} gives
\begin{align}
...
The solutions to \eqnref{BulkCoex} define the coexistance curve,
...
\subsubsection{The attractive perturbation}
...
\begin{figure}
 \centering
  \subfloat[]{\input{part_theoretical/nucleation/figs/c2_cnt_nucleation_rate.tex}}
  \caption{
...
The nucleation rates  (\centi\metre\rpcubed\reciprocal\second)  for \pfb,  perfluoropentane and water is plotted in \figref{cnt:rate}.
...
However, since \unit{25}\degreecelsius\ is also well below the critical temperature, % (\tabref{:nuc:critical_temps}), 
