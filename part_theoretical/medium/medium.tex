...
The predictive power of the linear model is expored in \secref{linearFluid}.
...
An alternative is suggested in \secref{incompressibleFluid},
where the dynamics of the fluid are modelled correctly with the Euler Equation,
but the equation of state is chosen in such a way as to enforce self-consistency.
...
but rather modifying the type of fluid probed.

Neither of these models are Lorentz invarient.
In \secref{Maxwell} this is corrected with the second model is extended to this regime.
...
\section{A linear medium}\label{sec:linearFluid}
...
\end{align}
Inserting into \eqnref{euclidianContinuityLinear} we obtain,
\sub{
    \begin{align}
 \rho_0  \frac{\partial u}{\partial t} &= - \del p \label{eqn:euclidianNSLinearTwo} \\
 \frac{\partial p }{\partial t} &=  - \rho_0 \frac{\partial p}{\partial \rho}  \del \cdot u. \label{eqn:euclidianContinuityLinearTwo}
\end{align}
}
...
\sub{
  ...
  \eqa{
     \vdel^2 u - \frac{1}{\frac{\partial p}{\partial \rho} }\frac{\partial^2 u}{\partial^2 t} &=  - \vdel \cross ( \vdel \cross u). 
 ...
 \del^2 p  - \frac{1}{\frac{\partial p}{\partial \rho} }\frac{\partial^2 p }{\partial^2 t}  &=  0
  }
  }
Defining the sound speed to be
\begin{align}
c^2 \equiv \frac{\partial p(\rho_0)}{\partial \rho}
\end{align}
we see that \eqnref{waveEuclid} defines a pair of wave equations.
...
\section{A linear {\em fluid} with constant sound speed}\label{sec:incompressibleFluid}
...
\begin{align}
  d p &= c^2 d\rho = \gamma p / \rho
\end{align}
so
\eqa{
  \frac{p}{p_0} &= \lr{\frac{\rho}{\rho_0}}^{\gamma}\\
  \frac{c}{c_0} &= \lr{\frac{\rho}{\rho_0}}^{\half(\gamma-1)}\\
}
...
The approach is not without problems.
The actual predicited speed of sound,
when $\gamma \rightarrow 1$ is not correct.
...
However, due to the importance of the section the derivation is repeated in tensor algebra in \appref{acousticSRTensor}.
