...
By using \eqnref{soundSpeedDiffNR} we write the continuity equation (equation \eqnref{}) in terms of pressure
\sub{
  \label{eqn:attemptTwo:wave:tmp}
\eqa{
  \frac{1}{\rho c_0}\lr{\frac{\partial p}{\partial t} + \vu \cdot \vdel p} +\vdel\cdot \vu \\
  \intertext{and use the identy $\vu \cross (\vdel \cross \vu) \equiv \vu \cdot \vdel \vu - \vdel \half ^2$
...
\eqa{
  ...
  \frac{\partial}{\partial t}\lr{\frac{1}{\rho c_0}\frac{\D p}{\D t} } - \vdel^2 w = \vdel \lr{\cdot \vu \cross \lr{\vdel \cross \vu}} 
}
...
By using equation \eqnref{thermoNR} and the momentum equation, \eqnref{}, the first term can be written
\eqa{
...
%}
%///
Inserting equations \eqnref{}, \eqnref{} and \eqnref{} back into equation \eqnref{} yeilds the final answer 
\eqa{
  \lr{\frac{1}{c_0^2}\frac{D^2}{Dt^2} - \frac{1}{\rho} \vdel \cdot \lr{\rho \vdel} } w = \frac{1}{\rho} \vdel\lr{\rho \vu \cross \vdel \cross \vu}. \label{eqn:attemptTwo:waveEqn}
}
...
where  $\Dt \equiv \dt + \vv \cdot \vdel$.\todo{fix the reference }
