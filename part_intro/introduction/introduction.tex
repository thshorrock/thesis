...
\hl{The lack of a contrast agent that is able to target a wide range of tissue types is a real loss}\todo{not true due to Doppler measuring blood flow - revise}.
%Its lack of functional imaging is a real loss.

There is a second problem that is caused by the lack of submicron bubbles.\todo{The disadvantage of imaging with high-frequency should be clarified}
...
If translational invariance is also assumed,  then acoustical measurements are  invariant to the Lorentz group. \todo{EM analogy is not proved}
...
\Chapref{measurement} applies the acoustical model by deriving how the pulsations of a bubble appear when imaged with ultrasound.
...
\Chapref{rationale} \todo{Check these chapter numbers are correct}gives the scope and rationale of the experimental design.
...
\todo{Take the first opportunity to clarify the key differences of the dual-frequency approach here with the literature}
\hl{The two wave, two frequency technique used in this thesis bares similarity to {\sc{Surf}} imaging that is already described in the literature{\cite{Hanson2009}}.
  In  {\sc{Surf}} imaging a low frequency \emph{manipulation} pulse is emitted with a high frequency imaging pulse.
  ...
  %The actual imaging technique used in {\sc{Surf}} imaging is pulse inversion.  
\hl{For the purpose of imaging two pulses are used.
  ...
  %The two pulses differ in the phase between the driving and imaging waves.
  To form an image, the high frequency components of the two waves are subtracted from each other.
  Therefore, the {\sc{Surf}} technique images the cumulative difference in the response of the medium to the manipulation wave.
...
\hl{The two wave technique used here differs from {\sc{Surf}}  in this final imaging step.
  ...
  The purpose of {\sc{Surf}} is to maximise the difference between two pulses with different relative phases.
  ...
  We would therefore expect {\sc{Surf}} imaging to give good contrast when the manipulation wave and bubble size are chosen to maximise this low frequency resonance.
