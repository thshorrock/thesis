\newcommand{\dbFifteen}{\unit{2.2}\mega\pascal}
\newcommand{\dbThirty}{\unit{0.4}\mega\pascal}
\newcommand{\dbThirtyTwo}{\unit{0.3}\mega\pascal}
...
%\subsection{A bubble generated from a perfluorocarbon emulsion}


%Each of these methods have their respective advantages.
\desc{
\item[A bubble generated from a perfluorocarbon emulsion:]\hfill \\
  ...
  has remained troublesome\cite{Burns2010,Lin2013}).
...
\begin{figure}[p]%[htp]
  ...
  \subfloat[Gray scale representation of phase sampling along co-axis]{
    \label{fig:phase_exp_b}
    \includegraphics[width=1.0\textwidth]{exp_theory.eps} %[width=.8\textwidth ]
    }
  \caption{     %A sanity check.  
...
\todo{Spatial axes in figure 6.5 and confirm wavelength}
\hl{
...
%
%The pressure profile measured is ref
%A hydrophone is used to deter relationship at different distances between the two transducers was then found by moving the hydrophone
% along the (approximate) co-axis of the two transducers.
The voltage-time waveforms from tracking the hydrophone across the focus are shown in \figref{phase_exp_a}.
600 traces over \unit{6}\milli\metre\ were taken and these are plotted in \figref{phase_exp_b}.  
...
It is seen from \figref{phases} that the entire phase space is sampled every microsecond of distance.  This is half the period of the low frequency wave,
which follows from the two waves moving towards each other. 
However, the separation between a $2\pi$ difference in phase in the echo will again be \unit{2}\micro\second.
An echo that samples the peak pressure of the driving wave will remain with the peak pressure on its return.%     echo will again match the period of the low frequency wave.
...
\todo{Comment on the tap water storage before the experiment}
In all cases the water used in this study was left covered for 6 weeks to equilibrate.
...
\todo{Discuss extrapolation error}
If the pressures are not too high then the relation between the voltage supplied to the transducer and the pressure of the focus can be inferred
from such plots by assuming the relation is linear.
At high pressures this relation breaks down due to the acoustic saturation of the medium\cite{Karzova2012}.
Such extrapolations are only carried out in the preliminary study in this thesis.
%However, at the pressures used in this thesis, 
%the acoustic saturation\cite{Karzova2012} of the medium breaks the linearity between supplied voltage and pressure and so such extrapolation is not done here.

\item The \unit{20}\mega\hertz\ transducer was a spare taken from a  Cortex Skin Scanner.
...
\begin{centering}
\begin{figure}
\subfloat[Driving transducer (0.5\mega\hertz) ]{
    \label{fig:exp:Dprofile}
    \includegraphics[width=.9\textwidth]{0_5mhz_profile.eps}}\\
...
\begin{figure}[h]
     \centering
          \includegraphics[width=0.8\textwidth]{prelim_arrangement.1}
...
The electronic arrangement of the preliminary experiment is summarised in \figref{exp:prelim:arrangement}.
...
{\label{fig:water_cavitation_70}
 \includegraphics[width=0.8\textwidth]{rotterdam_02_water_caption.png}}\\
 \subfloat[Water sample driven at \dbFifteen.  Both the imaging and the driving waves are on]
{\label{fig:water_cavitation_85}
 \includegraphics[width=0.8\textwidth]{C3water8500003_caption.png}}
\caption{The Hilbert transform of the imaging radio-frequency data, recovered when the imaging transducer is swung on a pendulum. 
The banding is seen at \unit{2}\micro\second\ intervals,
in accordance with the period of the driving wave.
 Please refer to the sub-caption as to whether the pulsing wave was on or not.}
...
\todo{Add spatial scales to \figref{water_cavitation_prelim} and confirm wavelength}
...
%\todo[inline]{I cannot think of other relevant questions that I could ask of my data?}
 ...
 \Figref{water_cavitation_85} plots the image that is obtained with a driving pressure of \dbFifteen.
...
\item[The location of the scattering bubble(s):]
  ...
  but \figref{exp:Dprofile} shows that the fall off in axial pressures is quite gradual.
  The excess pressure technique cannot be applied in this experiment to help determine the location of the bubble.
  This is because the imaging transducer is moving between adjacent alines,
  and because the imaging transducer fires for every A-line.
...
%each of which  can be detected by the imaging transducer.
